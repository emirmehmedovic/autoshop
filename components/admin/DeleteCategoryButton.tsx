...
export function DeleteCategoryButton({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : "Greška pri brisanju kategorije")
    } finally {
      setDeleting(false)
    }
  }

  // Disable delete if category has products
  ...
  return (
    <>
      <button
        ...
        disabled={!canDelete}
        className="p-2 text-red-400 hover:bg-red-400/10 rounded-lg transition border border-transparent hover:border-red-400/20 disabled:opacity-50 disabled:cursor-not-allowed"
        title={!canDelete ? "Kategorija ima proizvode" : "Obriši kategoriju"}
      >
        <Trash2 size={18} />
      </button>
            ...
            <div className="flex items-start gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-gray-400">
                  Da li ste sigurni da želite da obrišete <span className="font-bold text-white">{categoryName}</span>?
                </p>
